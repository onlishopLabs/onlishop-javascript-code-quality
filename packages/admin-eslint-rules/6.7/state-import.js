export default {
	meta: {
		...
		docs: {
			description:
				"Replace Onlishop.State with Onlishop.Store (and destructured State accordingly).",
			category: "Best Practices",
			recommended: false,
		},
		fixable: "code",
		schema: [],
		minOnlishopVersion: "6.7.0.0",
	},
	...
	create(context) {
		...
		function _isOnlishopState(node) {
			return (
				node &&
		...
		return {
			// Check for destructuring: const { State } = Onlishop;
						...
						computedText = props
							.map((prop) => {
								return (
									...
									baseIndent +
									"    " +
									`return Onlishop.Store.get('${storeName}').${prop};\n` +
									baseIndent +
									"}" +
							...
							.map((prop) => {
								...
								if (
									...
									return (
										...
										baseIndent +
										"    " +
										`return Onlishop.Store.get('${storeName}').${mappedProp};\n` +
										baseIndent +
										"},"
									);
								}
			...
			CallExpression(node) {
				if (
					...
					if (
						...
						stateMember.object.name === "Onlishop" &&
					...
					if (
						...
						if (parts.length === 2) {
							...
							const argsText = args
								...
								.join(", ");
							const newCode = `${shortHand ? "" : "Onlishop."}Store.get('${storeName}').${methodName}(${
								argsText ? argsText : ""
							})`;
							context.report({
								node,
								message:
									"Replace State.commit/Onlishop.State.commit call with Onlishop.Store.get call.",
								fix(fixer) {
