export default {
	meta: {
		...
		docs: {
			description:
				"Forbid passing `snippets` to Onlishop.Module.register or Module.register",
	...
	create(context) {
		return {
			CallExpression(node) {
				...
				// Determine if the call is from Onlishop.Module.register or (destructured) Module.register.
				let isOnlishopModuleRegister = false;
				if (callee.object.type === "MemberExpression") {
					// e.g. Onlishop.Module.register(...)
					const objectPart = callee.object;
					if (
						objectPart.object.type === "Identifier" &&
						objectPart.object.name === "Onlishop" &&
						objectPart.property.type === "Identifier" &&
						objectPart.property.name === "Module"
					) {
						isOnlishopModuleRegister = true;
					}
				} else if (callee.object.type === "Identifier") {
					// e.g. Module.register(...)
					// For simplicity we'll assume that if `Module` is used directly, it is the one from Onlishop.
					if (callee.object.name === "Module") {
						isOnlishopModuleRegister = true;
					}
				}

				if (!isOnlishopModuleRegister) {
					return;
				}
				...
				options.properties.forEach((prop) => {
					...
					if (
						...
						context.report({
							node: prop,
							message:
								"Passing 'snippets' to Onlishop.Module.register is forbidden as it increases the bundle size. Snippets are automatically loaded when they are placed in a folder named snippet.",
						});
					}
				});
			},
